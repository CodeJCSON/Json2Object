...
//根据字符串获取集合
+(id)getContainerByData:(NSString*)str
{
    NSError *error;
   id idContainer= [NSJSONSerialization JSONObjectWithData:[str dataUsingEncoding:NSUTF8StringEncoding] options:NSJSONReadingMutableContainers error:&error];
    if (error!=nil) {
        NSLog(@"错误是%@",error);
    }
    return idContainer;
